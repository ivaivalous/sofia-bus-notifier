...
class CgmInterator(object):
    ...
    __API_REFERER = (
        "{}/bg/transport/virtual-tables-by-line?"
        "type=bus&line={}&route=1&stop={}")
    ...
    def __get_twilio_phone_number(self):
        ...
        environment variable, and if missing, default to the
        __TWILIO_PHONE_NUMBER constant.
    ...
    def __build_headers(self):
        return {
            ...
            "Referer": self.__API_REFERER.format(
                self.__WEB_HOST, self.line_number, self.stop_number),
    ...
    def get_next_arrivals(self):
        ...
        response = requests.get(
            self.__ARRIVALS_API.format(
                self.__API_HOST, self.stop_number, self.line_number),
            headers=self.__build_headers())

        response = json.loads(response.text)
        if not response:
            # The response would be empty at times when the line is not running
            return
        ...
        if not response["lines"]:
            ...
            self.arrival_times = [
                t["time"] for t in response["lines"][0]["arrivals"]]
    ...
    def __repr__(self):
        if not self.last_timestamp:
            return "Arrivals for bus {} @ {}.".format(
